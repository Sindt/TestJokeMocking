...
public class JokeFetcher {
	 ...
	 * 
	 * @param jokeTokens.
	 *            Example (with valid values only):
	 ...
	 * 
	 * @param jokesToFetch
	 *            A comma separated string with values (contained in
	 *            availableTypes) indicating the jokes to fetch. Example:
	 *            "eduprog,chucknorris,chucknorris,moma,tambal" will return five
	 *            jokes (two chucknorris)
	 * @param timeZone.
	 *            Must be a valid timeZone string as returned by:
	 *            TimeZone.getAvailableIDs()
	 * @return A Jokes instance with the requested jokes + time zone adjusted
	 *         string representing fetch time (the jokes list can contain null
	 *         values, if a server did not respond correctly)
	 * @throws JokeException.
	 *             Thrown if either of the two input arguments contains illegal
	 *             values
	 */
	public Jokes getJokes(String jokesToFetch, String timeZone) throws JokeException {
		...
		String tzString = formatter.getFormattedDate(timeZone, new Date());
